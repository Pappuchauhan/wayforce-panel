...
const useFilter = (data) => {
  ...
  const [searchMyCategory, setSearchMyCategory] = useState("");
  ...
  const serviceData = useMemo(() => {
    ...
    if (searchMyCategory) {
      console.log(searchMyCategory)
      services = services.filter(
        (search) =>
          search?.name
            ?.toLowerCase()
            .includes(searchMyCategory.toLowerCase())  
      );
    }
    //Coupon filtering
    if (searchCoupon) {
  ...
  const handleSubmitMyCategory =(e)=>{
    e.preventDefault(); 
    setSearchMyCategory(userRef.current.value);
  }
